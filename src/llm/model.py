...
class GPT(nn.Module):
    ...
    def soft_loss(
        ...
        legal_mask: mx.array | None = None,
    ) -> mx.array:
        """Soft CE at last context position, softmax over legal actions only.

        legal_mask: (B, V) bool. Illegal logits are set to -1e9. Targets are
        zeroed off-mask and renormalized so 0 * -inf cannot NaN.
        """
        ...
        if legal_mask is not None:
            ninf = mx.array(-1e9, dtype=gathered.dtype)
            gathered = mx.where(legal_mask, gathered, ninf)
            zeros = mx.array(0.0, dtype=target_probs.dtype)
            target = mx.where(legal_mask, target_probs, zeros)
            target = target / mx.maximum(target.sum(axis=-1, keepdims=True), 1e-12)
