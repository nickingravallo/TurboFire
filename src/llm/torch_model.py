...
class GPT(nn.Module):
    ...
    def soft_loss(
        ...
        legal_mask: (B, V) bool. Illegal logits are set to -inf. Targets are
        ...
        if legal_mask is not None:
            gathered = gathered.masked_fill(~legal_mask, torch.finfo(gathered.dtype).min)
            target = target_probs * legal_mask.to(dtype=target_probs.dtype)
